...
function updateComment(url, request) {
  ...
  else  if (!id || !requestComment||!requestComment.id||!requestComment.body||!requestComment.username||!requestComment.articleId||id!=requestComment.id) {
      response.status = 400;
    } else {
      database.comments[id]=requestComment
...
function deleteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const savedArticle = database.comments[id];
  const response = {};

  if (savedArticle) {
  let remove1 = database.users[database.comments[id].username].commentIds.splice(id-1,1)
  let remove2=  database.articles[database.comments[id].articleId].commentIds.splice(id-1,1)
